...
var Lightbulb = class {
  async show(doc, only) {
    ...
    const range = await import_coc.window.getSelectedRange("cursor");
    if (!range)
      return false;
...
async function activate(extCtx) {
  ...
  const enableVirtualText = import_coc.workspace.isNvim && import_coc.workspace.nvim.hasFunction("nvim_buf_set_virtual_text") && config.get("enableVirtualText");
  ...
  const followDiagnostic = config.get("followDiagnostic");
  const {nvim} = import_coc.workspace;
  if (enableVirtualText)
    nvim.command("hi default LightBulbVirtualText guifg=#FDD164", true);
  if (enableSign) {
    nvim.command("hi default LightBulbSign guifg=#FDD164", true);
    nvim.command(`sign define LightBulbSign text=${signText.replace(" ", "")} texthl=LightBulbSign linehl=LightBulbSignLine`, true);
  }
  const nvim6 = import_coc.workspace.has("nvim-0.6.0");
  ...
  extCtx.subscriptions.push(import_coc.events.on(["CursorHold", "CursorHoldI"], async () => {
    ...
    const disabled = await buffer.getVar("coc_lightbulb_disable") == 1 || followDiagnostic && await buffer.getVar("coc_diagnostic_disable") == 1;
    ...
    if (enableVirtualText) {
      const lnum = (await import_coc.workspace.getCurrentState()).position.line;
      const chunks = [[virtualText, "LightBulbVirtualText"]];
      if (nvim6) {
